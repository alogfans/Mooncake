...
class DeviceQuota {
   ...
   public:
    ...
    struct FaultInjection {
        ...
        // Inject fault: disable specific device(s)
        // device_bitmask: bit N set means device N is disabled
        void disableDevices(uint64_t device_bitmask) {
            std::lock_guard<std::mutex> lock(mutex_);
            disabled_device_mask_ |= device_bitmask;
        }

        // Enable specific device(s)
        void enableDevices(uint64_t device_bitmask) {
            std::lock_guard<std::mutex> lock(mutex_);
            disabled_device_mask_ &= ~device_bitmask;
        }

        // Get current fault mask
        uint64_t getDisabledMask() const {
            return disabled_device_mask_.load(std::memory_order_relaxed);
        }

        // Set fault mode ("disconnect", "half_speed")
        void setFaultMode(const std::string& mode) {
            std::lock_guard<std::mutex> lock(mutex_);
            fault_mode_ = mode;
        }

        const std::string& getFaultMode() const {
            return fault_mode_;
        }
        ...
        void clear() {
            std::lock_guard<std::mutex> lock(mutex_);
            disabled_device_mask_.store(0, std::memory_order_relaxed);
            fault_mode_.clear();
        }

    private:
        std::atomic<uint64_t> disabled_device_mask_{0};
        std::string fault_mode_;
        std::mutex mutex_;
    };

    // Public API for fault injection
    static void injectDeviceFault(uint64_t device_bitmask) {
        FaultInjection::instance().disableDevices(device_bitmask);
    }

    static void clearDeviceFault(uint64_t device_bitmask) {
        FaultInjection::instance().enableDevices(device_bitmask);
    }

    static void setFaultMode(const std::string& mode) {
        FaultInjection::instance().setFaultMode(mode);
    }

    static uint64_t getFaultMask() {
        return FaultInjection::instance().getDisabledMask();
    }
