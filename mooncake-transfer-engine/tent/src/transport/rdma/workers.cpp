...
std::shared_ptr<RdmaEndPoint> Workers::getEndpoint(Workers::PostPath path) {
    ...
    if (target_id != LOCAL_SEGMENT_ID) {
        rpc_server_addr = hint.segment->rpc_server_addr;
    }
...
void Workers::disableEndpoint(RdmaSlice* slice, int ibv_wc_status) {
    ...
    if (auto ep = slice->ep_weak_ptr.lock()) {
        ep->context().endpointStore()->remove(ep.get());
    }
}
...
void Workers::asyncPollCq() {
    ...
    for (int index = 0; index < num_contexts; index++) {
        ...
        for (int i = 0; i < nr_poll; ++i) {
            ...
            auto ep = slice->ep_weak_ptr.lock();
            double enqueue_lat = 0, inflight_lat = 0;
            if (slice->enqueue_ts > 0)
                enqueue_lat = (poll_ts - slice->enqueue_ts) / 1000.0;
            if (slice->post_send_end_ts > 0)
