...
static uint64_t delayedRedispatchBackoffNs(Transport::Slice *slice,
    ...
    const uint64_t jitter_window =
        std::max<uint64_t>(1, delay_ns / 4);
...
void WorkerPool::delayRedispatch(Transport::Slice *slice,
    ...
    if (rail_pause_remaining_ns > 0) {
        const uint64_t pause_jitter_ns = delayedRedispatchJitterNs(
            slice, kDelayedRedispatchMinBackoffNs);
        delay_ns = std::max(delay_ns, rail_pause_remaining_ns + pause_jitter_ns);
    }
    ...
    while (workers_running_.load(std::memory_order_relaxed)) {
        ...
        if (next_delayed_due_ns > 0) {
            ...
            if (next_delayed_due_ns > now) {
                const uint64_t sleep_ns = std::min(
                    next_delayed_due_ns - now, kDelayedRedispatchWakeCapNs);
...
uint64_t WorkerPool::peerRailPauseRemainingNs(
    const std::string &peer_nic_path, uint64_t now) {
